...
scenarios <- read.csv("./Data/GDP_scenario.csv")
scenarios$Date <- as.Date(paste(scenarios$Date,"-01",sep=""))
scenarios
scenario1 <- scenarios$scenario_1
scenario2 <- scenarios$scenario_2
scenario3 <- scenarios$scenario_3

import_forecast <- read.csv("./Data/Import_forecast.csv")
import_forecast$Date <- as.Date(paste(import_forecast$Date,"-01",sep=""))
import_forecast
forecast1 <- import_forecast$scenario_1
forecast2 <- import_forecast$scenario_2
forecast3 <- import_forecast$scenario_3
...
```{r Import trimming}
ts_EU_Import_201407_202203 <- ts_import,frequency=12,start=c(2014,07))
ts_EU_GDP_train<-subset(ts_EU_GDP_201407_202203,end=length(ts_EU_GDP_201407_202203)-n_for)
ts_EU_GDP_test<-subset(ts_EU_GDP_201407_202203,start=length(ts_EU_GDP_201407_202203)-n_for)
```
...
ARIMA_Four_for_withGDP1_202212 <- forecast::forecast(ARIMA_Four_fit_withGDP_202212,
                           xreg=ts_EU_GDP1_201407_202301[81:93],
                           h=9
                           ) 
ARIMA_Four_for_withGDP2_202212 <- forecast::forecast(ARIMA_Four_fit_withGDP_202212,
                           xreg=ts_EU_GDP2_201407_202301[81:93],
                           h=9
                           ) 
ARIMA_Four_for_withGDP3_202212 <- forecast::forecast(ARIMA_Four_fit_withGDP_202212,
                           xreg=ts_EU_GDP3_201407_202301[81:93],
                           h=9
                           ) 
autoplot(ts_EU_price_shorten_201407_202203) +
  autolayer(ARIMA_Four_for_withGDP1_202212, series="ARIMA_GDP1",PI=FALSE) +
