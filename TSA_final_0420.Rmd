...
**Introduction, Motivation, Relevance, and Objectives**
  Since the start of the Russian invasion of Ukraine, although the volume of Russian gas delivered to the EU through Ukraine did not actually decrease, natural gas spot prices and futures prices rose considerably. (Reuters, 2022) For example, “in the UK market, the March price was up 58.6% at 321.97 pence per therm and the winter 2022 price had risen 37.85% to 105p/therm.” One reason is that Russia’s overall natural gas supply decreased compared to previous years. (Reuters, 2022) Another reason is that the demand for natural gas in the EU is rising due to the economic recovery from the pandemic. Arguably, the most important reason that EU natural gas price increase is due to market speculation in response to the political uncertainties. This gives us the motivation for our project: to forecast the EU natural gas price after the Ukraine crisis.
  ...
  The focus of our analysis is on the pipeline transmission of natural gas from Russia to the EU countries. In a 2010 study, “Russia is the world’s largest exporter of pipeline gas, accounting for 26% of global exports”. Many EU countries are among the top 10 gas importers, including Germany, Italy, France, Spain, and the UK (formal EU member). This large volume of Russian export and EU imports of natural gas highlight the dependence of the EU on Russia's imported natural gas through pipelines.
  
  The objective of this project is to forecast the EU natural gas price after the Ukraine crisis. The motivation is to see which model (if any), can predict natural gas prices in a period of high volatility caused by political events. We want to explore whether these exogenous variables can capture some of the impacts of the Russia-Ukraine war, thus improving our model performance. 

Specifically, these are our exogenous variables:
  Consumption: it reflects the seasonal variations in NG price (higher in the winter when heating need is higher)
  GDP: it is a proxy of economic activities (high economic activities will need more energy inputs and more NG). 
  Imports: EU’s high dependence on imported NG from Russia

These are our hypotheses:
  1)Forecasting in a period of high uncertainty will be difficult, as the historical observations are not very applicable to the testing period
...
table_1
```
  To better perform the forecast, we introduce three exogenous variables: conusmption, import, and GDP(as a proxy of economic status) in the EU. Historical monthly natural gas price and the monthly normalized GDP in Europe are from Federal Reserve Banks of St.Louis’ website, and the monthly natural gas consumption and import in the EU are from the Eurostat data browser. Through exploring the correlations between natural gas price and the other factors, we found that while all variables have weak correlations, in which GDP shows a positive correlation to NG price and import shows a negative correlation, and consumption with really weak correlation (Table 1). Therefore, we decided to include GDP and natural gas imports as exogenous variables in our forecast. As GDP and import shows a strongest positive correlation of 0.05, we also analyze models that include both variables to explore how their interaction can impact the forecast result.
...
**Method**
  Our method is to fit ARIMA, Neural Netowrk (NN), STL+ETS, and TBATS models on NG price training set (including models with exogenous variables), and use the fitted to models to forecast NG price from January 2022 to March 2022. Then the forecasted results are compared to the observed data and the accuracy of different models are compared. Then, a best model with the lowest RMSE score will be selected to forecast the NG price from April 2022 to December 2022. If the model allow us to add in exogenous varaibles, we would add in proposed scenarios for GDP and import to generate corresponding forecast results. 

**Result**
 Graphs below show the comparison between forecasted price from 2022/1 to 2022/3 using models that fitted the training set and the observed data.
```{r, echo=FALSE}
#Neural Network Test
...
autoplot(ts_EU_price_shorten_201407_202203,main="Figure 6. Forecasted EU NG Price with NN Model") +
  ...
  ylab("EU Natural Gas Price ($/MMBtu)") 
```
  According to the best performed model, which is NN purely dependent on NG historical price, the forecast results show that EU NG price will fluctuates a lot throughout the year with an overall decreasing trend. The peak is forecasted to be in March 2022, and the price will falls back to 28 $/MMBtu at the end of the year (Figure 6). 
...
autoplot(ts_EU_price_shorten_201407_202203, main=" Figure 8. EU NG Price Forecast with NN under Import Scenarios") +
  ...
  ylab("EU Natural Gas Price ($/MMBtu)") 
```
  NN with import as a regressor forecasts a more significant decreasing trend of NG price over 2022, even though at the end of the year, the price is forecasted to be 30 $/MMBtu (similar as previous NN models). The lowest price forecast reaches almost 20 $/MMBtu. There is no significant difference between the three proposed import scenarios (Figure 8).
...
autoplot(ts_EU_price_shorten_201407_202203, main="Figure 11. EU NG Price Forecast Results under 3 GDP Scenarios") +
  ...
  ylab("EU Natural Gas Price ($/MMBtu)")
```
  The ARIMA model with GDP as a regressor shows a similar trend, leading to a slightly lower price forecast at the end of the year (25$/MMbtu) than the original ARIMA model (30$/MMBtu), and the forecast is still with high uncertainaty (i.e. wide confidence interval). There is no significant forecast differences under the three scenarios we proposed, and neither of the three scenarios cause a significant change in the forecast result compared to the original ARIMA model (Figure 11).
...
autoplot(ts_EU_price_shorten_201407_202203,main="Figure 12. EU NG Price Forecast Results under 3 Import Scenarios") +
  ...
  ylab("EU Natural Gas Price ($/MMBtu)")
```
  ARIMA models with import as a regressor show completely different results than the previous models. There are much smaller fluctuations and the overall trend is going upward, leading to a NG price forecast at around 45 $/MMbtu at the end of 2022.The much narrower confidence interval indicates that the integration of import data improves the uncertainty of the price forecast. Three import scenarios lead to similar trends but slightly off level of forecasts. Import decrease by 5% returns the highest NG price forecast, while import decrease by 20% returns the lowest NG price forecast (Figure 12).
  Even though historical import and NG price data are negatively correlated, we found that the scenario with lowest import decrease (5%) leads to lowest NG price forecast, and the highest import decrease (20%) leads to highest NG price forecast (Figure x). This indicates that in highly fluctuated time series, the already weak correlation between two variables might be polluted, thus the forecast results might not show the same correlation relationship shown in the historical data.
...
2)	As expected, Neural network with the lowest RMSE score at 7.54, turned out to be the best performed model among all models in capturing the nonlinear trend in our time series. 
3) Adding exogenous variables doens't necessarily lead to an imporvement in model performance. From our results, ARIMA models performed better with exogenous variables, while NN models performed worse with exogenous variables.
Interestingly, we found that some exogenous variables can improve the certainty of the forecast while others cannot. For example, adding import data into ARIMA model greatly narrowed the confidence level while adding GDP led to a slightly shift in confidence interval but hte width keeps the same. In conclusion, adding exogenous variables does not necessarily decrease the RMSE, but may improve the certainty of the forecast. 
4) The variable has stronger correlation with NG price indeed affects the forecast results more, but it doesn't necessarily improve the forecast performance. In both NN and ARIMA models, import has a greater impact on NG price forecast results. Import data also leads to greater change in RMSE compared to the ARIMA and NN without any exogenous regressors, but does not necessarily lower the RMSE score. 