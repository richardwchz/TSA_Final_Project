...
print(pettittTest2)
```
#By using the pettitt.test function, we found two inflection points in our natural gas price data, one in 2004 and the other one in 2014, meaning there is no significant change in the trend after 2014. In order to correctly forecast the natural gas price in 2022, we chose to only use data from 2014.07 to 2021.12 for our analysis. Although the natural gas price data for the first three month of 2022 is available, we chose not to use it to generate our forecast, since Russia started mobilizing its troops near Ukraine at the beginning of 2022, causing tension that has already driven up the natural gas price.
