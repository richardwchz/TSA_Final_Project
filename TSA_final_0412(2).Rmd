...
print(pettittTest2)
```
#By using the pettitt.test function, we found two points of inflection in our historical natural gas price data, one in September 2004 and the other one in July 2014, which means that there is no significant change in the trend after 2014. Because Russia started mobilizing its troops near Ukraine at the beginning of 2022, we assume that the impact of Ukraine crisis on natural gas price started in January 2022.
...
df_cor<-cor(df_cor)
row.names(df_cor) <- c("NG Price", "GDP", "Consumption", "Import")
colnames(df_cor) <- c("NG Price", "GDP", "Consumption", "Import")
table_1 <- kable(df_cor,"pipe", caption = "Table 1. Correlation Coefficients Matrix")
table_1
```
#To understand the degree of this impact, we collected the historical monthly natural gas price and the monthly normalized GDP in Europe from Federal Reserve Banks of St.Louis’ website, and the monthly natural gas consumption and import in the EU from the Eurostat data browser. Through exploring the correlations between natural gas price and the other factors, we found that GDP and imports have much stronger correlations than consumptions (Table 1). Therefore, we decided to use GDP and natural gas imports as exogenous variables for our ARIMA model.
